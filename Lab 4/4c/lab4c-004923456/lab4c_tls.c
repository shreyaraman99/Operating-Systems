...
int main(int argc, char** argv) {
	...
	if(SSL_connect(ssl) != 1) {
		perror("Error: SSL Connect");
		exit(2);
	}

	char ibuf[20];
	sprintf(ibuf, "ID=%s\n", id);
	SSL_write(ssl, ibuf, strlen(ibuf));
	if(logFlag)
		dprintf(fd, "%s", ibuf);
	...
	while(1) {
		...
		if (!stopFlag) {
			...
			memset(bufse, 0, 20);
			sprintf(bufse, "%s %.1f\n", tString, tempCurrent);
			SSL_write(ssl, bufse, strlen(bufse));
			if(logFlag)
				dprintf(fd, "%s %1.f\n", tString, tempCurrent);
		}
		...
		while(difftime(tEnd, tStart) < period) {
			...
			if(pollInput[0].revents & POLLIN) {
				...
				memset(rbuf, 0, 256);
				int bRead;
				if((bRead = SSL_read(ssl, rbuf, 256)) < 0) {
